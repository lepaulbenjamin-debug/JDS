 ...
 * pour une estimation, une liste pour un classement ou une rafale. Le relais ne
 * connaît pas les types de manche et n'a pas à les connaître — il vérifie
 * seulement que ça reste petit. C'est la régie qui validera la forme.
 */
export async function pushAnswer(code, answer) {
  const db = getStore();
  const reponse = answer?.reponse;
