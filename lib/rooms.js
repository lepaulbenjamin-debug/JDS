// Relais de salons pour Quiz entre amis.
//
 ...
 *
 * `reponse` est volontairement opaque ici : un index pour un QCM, un nombre
 * pour une estimation, une liste pour un classement ou une rafale, un titre
 * tapé au clavier pour un mix. Le relais ne connaît pas les types de manche et
 * n'a pas à les connaître — il vérifie seulement que ça reste petit. C'est la
 * régie qui validera la forme.
 *
 * Ce filtre est le seul endroit du relais qui puisse faire disparaître une
 * réponse sans rien dire : une forme non prévue devient `null`, et la régie
 * compte le joueur absent. C'est ce qui est arrivé au mix, dont les réponses
 * sont du texte — d'où la règle, désormais : tout type ajouté ici doit avoir
 * son test dans `check-quiz.mjs`.
 */
export async function pushAnswer(code, answer) {
  const db = getStore();
  const brut = answer?.reponse;
  // Une chaîne est tronquée plutôt que rejetée : la borne du relais n'est pas
  // une règle du jeu, et perdre la réponse vaudrait bien pire que la raccourcir.
  const reponse = typeof brut === 'string' ? brut.slice(0, 120) : brut;
  const acceptable = reponse === null
    || typeof reponse === 'number'
    || typeof reponse === 'boolean'
    || typeof reponse === 'string'
    || (Array.isArray(reponse) && reponse.length <= 16);

  const ok = await db.appendAnswer(code, {
    playerId: String(answer?.playerId ?? '').slice(0, 40),
    round: Number(answer?.round) || 0,
    reponse: acceptable ? reponse : null,
    // Une tentative sur le fil rouge ne dépend d'aucune manche : elle passe par
    // le même tuyau, et la régie fera le tri.
    fil: typeof answer?.fil === 'string' ? answer.fil.slice(0, 60) : null,
    // L'annonce de niveau d'un « tu te mets combien ? », qui précède la réponse
    // au lieu de l'accompagner. Même tuyau, même principe : la régie borne.
    niveau: Number.isFinite(Number(answer?.niveau)) ? Number(answer.niveau) : null,
    // Le bulletin d'un joueur sur une proposition que l'appli n'a pas reconnue.
    // Deux champs, tous deux bornés : l'identifiant de l'auteur jugé, et oui ou
    // non. La régie vérifiera que ce candidat existe et que le votant a le droit.
    vote: answer?.vote
      ? {
          candidat: String(answer.vote.candidat ?? '').slice(0, 40),
          oui: Boolean(answer.vote.oui),
        }
      : null,
    joker: typeof answer?.joker === 'string' ? answer.joker.slice(0, 16) : null,
    // Qui le vol ou le sabotage vise. Sans cette ligne, le relais laisserait
    // passer le joker et jetterait sa cible en silence : tout le monde
    // saboterait le premier au classement, et personne ne comprendrait pourquoi.
    cible: typeof answer?.cible === 'string' ? answer.cible.slice(0, 64) : null,
