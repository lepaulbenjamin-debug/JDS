...
// Là où arrivent les messages. Dans l'environnement pour que ça change sans
// redéployer, avec une valeur par défaut pour que le formulaire marche dès le
// premier déploiement.
//
// Plusieurs adresses séparées par des virgules, et c'est volontaire : un
// message d'assistance ne doit pas dépendre du filtre anti-spam d'une seule
// boîte. On l'a appris en direct — les messages étaient acceptés par le serveur
// du destinataire et n'arrivaient nulle part, pendant que les mêmes envois
// atterrissaient en boîte de réception ailleurs. Deux adresses chez deux
// fournisseurs différents, et la question ne se pose plus.
const destinataires = () => (process.env.QUIZROOM_CONTACT_A || 'benjamin@creawebconseil.fr')
  .split(',').map((adresse) => adresse.trim()).filter(Boolean);
...
export async function envoyerUnMessage(champs, { ip = '', envoi = null } = {}) {
  ...
  if (envoi) return envoi({ email, nom, message, a: destinataires() });
  if (!cle || !expediteur) {
