...
import { verserLesPacks } from './packs.js';
 ...
 */
export async function verifierJetonTiers(jeton, { jwksUrl, emetteurs, audiences, cles = null }) {
  ...
  // `cles` n'est fourni que par les tests : la vérification de signature est le
  // seul rempart de ces deux chemins de connexion, et un test qui ne peut pas
  // l'exercer ne vérifie rien.
  const jwk = (cles ?? await clesDe(jwksUrl)).find((k) => k.kid === tete.kid);
...
export async function ouvrirParApple(jeton, nom, nonce = null, cles = null) {
  ...
  // Le nonce lie le jeton à CETTE demande de connexion : sans lui, un jeton
  // capté ailleurs et encore valide ouvrirait une session ici. L'appareil passe
  // sa valeur brute, Apple a reçu son empreinte — on recompose.
  if (nonce && corps.nonce !== empreinte(nonce)) throw erreur(403, 'Jeton rejoué.');
...
export async function ouvrirParGoogle(jeton, nonce = null, cles = null) {
  const corps = await verifierJetonTiers(jeton, {
  ...
  // Google renvoie le nonce tel qu'on le lui a donné, sans le hacher.
  if (nonce && corps.nonce !== nonce) throw erreur(403, 'Jeton rejoué.');
  const compte = await compteDeLIdentite({
    fournisseur: 'google',
    sujet: corps.sub,
...
export async function handleCompteRequest({ method, query, body, headers }) {
  ...
  try {
    ...
    if (method === 'POST') {
      ...
      if (action === 'apple') {
        return { status: 200, body: await ouvrirParApple(body?.jetonApple, body?.nom, body?.nonce) };
      }
      if (action === 'google') {
        return { status: 200, body: await ouvrirParGoogle(body?.jetonGoogle, body?.nonce) };
      }
      ...
      if (action === 'licence') {
        // La licence du compte, c'est son identifiant : un achat suit donc le
        // compte et non l'appareil. Ce que cet appareil avait déjà acheté est
        // versé au passage, sans être retiré d'ici — se déconnecter ne doit
        // jamais faire disparaître un pack payé.
        const verses = await verserLesPacks(body?.licence, compte.id);
        return { status: 200, body: { licence: compte.id, verses } };
      }
      if (action === 'vues') {
